...
class ReservaEnCurso:
    ...
    en console/playground queda en None. reserva_id queda en None hasta que
    crear_reserva guarda con exito; finalizar_llamada (tools/call.py) lo usa
    para no dejar que el agente cuelgue sin haber confirmado nada.
    """
...
async def crear_reserva(
    ...
    # No se reserva a ciegas: consultar_disponibilidad guarda en userdata las
    # fechas que reviso, asi que si no coinciden con las de aqui es que el
    # agente se salto la consulta (o el cliente cambio de fechas despues de
    # consultarlas). Es un error esperable, no una excepcion: el LLM lee el
    # mensaje, llama la tool que falta y reintenta en el mismo turno.
    reserva = ctx.userdata
    if (reserva.fecha_entrada, reserva.fecha_salida) != (fecha_entrada, fecha_salida):
        return {
            "success": False,
            "message": (
                "Antes de crear la reserva tienes que consultar disponibilidad "
                "con consultar_disponibilidad para esas fechas exactas."
            ),
        }
    ...
    ctx.userdata.reserva_id = reserva_id
